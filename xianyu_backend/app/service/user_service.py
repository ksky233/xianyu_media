...
from app.schemas.user_schema import UserLogin, UserChangePassword
...
class UserService:
    ...
    async def authenticate_user(self, login_data: UserLogin) -> Optional[User]:
        ...
        if not user:
            ...
            raise BusinessError(ErrorType.UNAUTHORIZED_ERROR, "用户不存在")
        
        # 检查用户是否被软删除
        if not user.is_active:
            logger.warning(f"用户登录失败，用户已被禁用: {login_data.username}")
            raise BusinessError(ErrorType.UNAUTHORIZED_ERROR, "用户已被禁用")
        
        # 验证密码
        if not self.verify_password(login_data.password, user.hashed_password):
            logger.warning(f"用户登录失败，密码错误: {login_data.username}")
            raise BusinessError(ErrorType.UNAUTHORIZED_ERROR, "密码错误")
